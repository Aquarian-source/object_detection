...
void Object_Detection::getPosition(float pos_x, float pos_y, float &resOut0, float& resOut1, float& dist, Mat frame)
{
	...
	/*
	warpPerspective(frame, O, M, Size(400,600));
	...
	*/
	//Linear ttransformation of geometries
...
void Object_Detection::postProcessing(Mat& frame, const vector<Mat>& outs)
{
	...
	/*
	void drawBoundingBox(Mat& frame, int class_id, vector<String> classes, int x, int y, int x_plus_w, int y_plus_h)
	{
		String label = static_cast<string> (classes[class_id]);
		//  double color = COLORS[class_id];
		rectangle(frame, (x, y), (x_plus_w, y_plus_h), 255, 2);
		int a = x - 10;
		int b = y - 10;
		putText(frame, label, { a, b }, FONT_HERSHEY_SIMPLEX, 0.5, 255, 2);
	}
	*/
	...
	for (size_t i = 0; i < indices.size(); ++i)
	{
		...
		drawBoundingBox(classIds[idx], confidences[idx], box.x, box.y, box.x + box.width, box.y + box.height, frame);
	}
	*/
}
